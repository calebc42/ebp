...
reports user interactions back as semantic events. The contract deliberately
does not name a programming language, UI framework, design system, or renderer
architecture. Any endpoint that implements the wire contract is conformant.
...
| [`contract.json`](contract.json) | The machine-readable projection (format 8) a renderer or authoring tool validates emissions against — the core node registry, per-node key schemas with universal attributes, the nested semantics envelope and node-derived defaults, field types and enums, the full method table (sender, class, legal states, capability gate, params, result, errors), the error-code registry, limits, the discriminated action schema, and the capability/trigger/state registries. Concrete renderer extensions are endpoint-owned and are not registered here. |
| [`goldens/`](goldens/) | The conformance corpus. JSON-line files: `widgets.golden` (every node type plus bare actions), `frames.golden` (one message per registry method, both directions), `hypertext.golden` (document node arrays), `editor.golden` (§19 splice known-answers — positions in Unicode scalar values per §19.1, with astral-plane and refused-splice cases that fail any UTF-16 code-unit or grapheme indexing). `goldens/wire/` holds byte-exact framed fixtures per §24.5 — positive vectors (the §9.3 known-answer handshake, a UTF-8 byte-count body, back-to-back frames) and negative vectors (§24.6 items 1–3) — described by `goldens/wire/manifest.json`. |
...
- **`protocol_version`** (offered in `session.hello`) — the wire version.
  Bumped only on a *breaking* change per SPEC §25. Currently `3`: the renderer
  extension line (amendment #178); `2` is the retired JSON-RPC envelope line
  and `1` is the retired NDJSON line.
- **`spec_version`** — the governing spec's revision (currently
  `3.1.0-draft`). Grows with additive, negotiated features and the node
...
- Reference implementations conformance-test against this contract by
  regenerating their own projections of `contract.json` and comparing the
  protocol goldens byte-for-byte.
